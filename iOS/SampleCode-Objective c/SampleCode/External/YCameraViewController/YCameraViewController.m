...
- (IBAction)donePhotoCapture:(id)sender{
    ...
    if (_didFinishPickingImageHandler) {
        _didFinishPickingImageHandler(self.captureImage.image);
    }
    
    if ([delegate respondsToSelector:@selector(didFinishPickingImage:Controller:)]) {
        [delegate didFinishPickingImage:self.captureImage.image Controller:self];
    }
