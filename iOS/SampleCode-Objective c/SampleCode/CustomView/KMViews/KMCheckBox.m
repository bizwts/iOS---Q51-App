...
#define kKMCheckboxMaxFontSize 100.0

//Custom Checkbox View
@interface KMCheckView : UIView

@property (nonatomic, retain) KMCheckBox *checkbox;
@property (nonatomic, assign) BOOL selected;

@end

@implementation KMCheckView
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (checkbox.checkState == KMCheckboxStateUnchecked) {
    ...
    if (checkbox.checkState == KMCheckboxStateUnchecked) {
        //Do Nothing
    } else if (checkbox.checkState == KMCheckboxStateChecked) {
        [checkColor setFill];
        [[checkbox getDefaultShape] fill];
    } else if (checkbox.checkState == KMCheckboxStateMixed) {
...
@interface KMCheckBox ()

@property (nonatomic, assign) CGRect boxFrame;

@end

@implementation KMCheckBox
{
    KMCheckView *checkView;
    UIColor *labelColor;
}
...
- (id)init {
    self = [self initWithFrame:CGRectMake(0, 0, KMCheckboxDefaultHeight, KMCheckboxDefaultHeight)];
    return self;
}
...
- (void)awakeFromNib {
    ...
    _checkAlignment = KMCheckboxAlignmentRight;
    _checkState = KMCheckboxStateUnchecked;
    _enabled = YES;
    checkView = [[KMCheckView alloc] initWithFrame:CGRectMake(self.frame.size.width - ((kBoxSize + kCheckHorizontalExtention) * self.frame.size.height), 0, ((kBoxSize + kCheckHorizontalExtention) * self.frame.size.height), self.frame.size.height)];
...
{
    self = [self initWithFrame:CGRectMake(0, 0, 100.0, KMCheckboxDefaultHeight)];
    if (self) {
        ...
        labelSize = [title sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:_titleLabel.font, NSFontAttributeName, nil]];
#else
...
- (void)autoFitFontToHeight
{
    ...
    CGFloat fontSize = kKMCheckboxMaxFontSize;
    ...
    do {
        ...
        CGSize labelSize;
#if  __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
        labelSize = [@"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:font, NSFontAttributeName, nil]];
#else
...
- (void)autoFitWidthToText
{
    CGSize labelSize;
#if  __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
    labelSize = [_titleLabel.text sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:_titleLabel.font, NSFontAttributeName, nil]];
#else
...
- (void)layoutSubviews
{
    if (_checkAlignment == KMCheckboxAlignmentRight) {
...
- (void)setCheckState:(KMCheckboxState)checkState{
...
- (void)setEnabled:(BOOL)enabled
{
    if (enabled) {
        ...
        float r, g, b, a;
        [labelColor getRed:(CGFloat *)&r green:(CGFloat *)&g blue:(CGFloat *)&b alpha:(CGFloat *)&a];
...
- (void)setCheckAlignment:(KMCheckboxAlignment)checkAlignment
{
...
- (id)value
{
    if (self.checkState == KMCheckboxStateUnchecked) {
        return uncheckedValue;
    } else if (self.checkState == KMCheckboxStateChecked) {
        return checkedValue;
    } else {
        return mixedValue;
    }
